// This module defines interface to the QsciAPIsEx class.
//
...
class QsciAPIsEx : public QsciAbstractAPIs
{
    Q_OBJECT

public:
    //! Constructs a QsciAPIsEx instance attached to lexer \a lexer.  \a lexer
    //! becomes the instance's parent object although the instance can also be
    //! subsequently attached to other lexers.
    QsciAPIsEx(QsciLexer *lexer);

    //! Destroy the QsciAPIsEx instance.
    virtual ~QsciAPIsEx();
...
protected:
    ...
    QsciAPIsEx(const QsciAPIsEx &);
    QsciAPIsEx &operator=(const QsciAPIsEx &);
};
