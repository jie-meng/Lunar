// This module implements the QsciAPIsEx class.
//
...
//    QMap<QString, QsciAPIsEx::WordIndexList> wdict;
...
{
public:
    QsciAPIsWorker(QsciAPIsEx *apis);
...
QsciAPIsWorker::QsciAPIsWorker(QsciAPIsEx *apis)
...
void QsciAPIsWorker::run()
{
    ...
    for (int a = 0; a < prepared->raw_apis.count(); ++a)
    {
        ...
        for (int w = 0; w < words.count(); ++w)
        {
            ...
            QsciAPIsEx::WordIndexList wil = prepared->wdict[word];
            ...
            wil.append(QsciAPIsEx::WordIndex(a, w));
...
// The ctor.
QsciAPIsEx::QsciAPIsEx(QsciLexer *lexer)
...
bool QsciAPIsEx::event(QEvent *e)
{
    switch (e->type())
    {
    case WorkerStarted:
...
bool QsciAPIsEx::load(const QString &filename)
{
    QFile f(filename);
...
void QsciAPIsEx::add(const QString &entry)
{
    apis.append(entry);
}
...
void QsciAPIsEx::remove(const QString &entry)
{
...
// supplied context.
QStringList QsciAPIsEx::positionOrigin(const QStringList &context, QString &path)
{
...
bool QsciAPIsEx::originStartsWith(const QString &path, const QString &wsep)
{
...
void QsciAPIsEx::updateAutoCompletionList(const QStringList &context,
        QStringList &list)
{
    QString path;
...
const QsciAPIsEx::WordIndexList *QsciAPIsEx::wordIndexOf(const QString &word) const
{
    QString csword;
...
void QsciAPIsEx::lastCompleteWord(const QString &word, QStringList &with_context, bool &unambig)
{
    // Get the possible API entries if any.
...
void QsciAPIsEx::lastPartialWord(const QString &word, QStringList &with_context, bool &unambig)
{
    if (lexer()->caseSensitive())
    {
...
void QsciAPIsEx::autoCompletionSelected(const QString &selection)
{
...
void QsciAPIsEx::addAPIEntries(const WordIndexList &wl, bool complete,
...
QStringList QsciAPIsEx::callTips(const QStringList &context, int commas,
...
bool QsciAPIsEx::enoughCommas(const QString &s, int commas)
{
    int end = s.indexOf(')');
...
void QsciAPIsEx::cancelPreparation()
{
    deleteWorker();
}
...
bool QsciAPIsEx::isPrepared(const QString &filename) const
{
...
bool QsciAPIsEx::loadPrepared(const QString &filename)
{
    QString pname = prepName(filename);
...
bool QsciAPIsEx::savePrepared(const QString &filename) const
{
...
QString QsciAPIsEx::defaultPreparedName() const
{
...
QString QsciAPIsEx::prepName(const QString &filename, bool mkpath) const
{
...
QStringList QsciAPIsEx::installedAPIFiles() const
{
